...
UPDATE_URL = os.environ.get("UPDATE_URL", "https://cross-sell-api-542s.onrender.com/data/update")  # update endpoint for table edits
...
def load_main_data() -> pd.DataFrame:
    if not API_URL:
        st.warning("API_URL is not configured.")
        return pd.DataFrame()
    try:
    ...
    if client_code_input:
        if display_df.empty:
            ...
            if st.button("Apply Change"):
                ...
                try:
                    if not UPDATE_URL:
                        st.error("UPDATE_URL is not configured.")
                        return
